...
// ✅ Fix default marker icons (important for React + Leaflet)
...
const LeafletMap: React.FC<AirQualityMapProps> = ({ zones }) => {
  const [mapCenter] = useState<[number, number]>([-1.2921, 36.8219]); // Nairobi lat/lng
    ...
    <Paper sx={{ p: 2, height: 600 }}>
      ...
      >
        ...
        {zones.map((zone, index) => {
          // ✅ Default coords fallback (GeoJSON uses [lng, lat])
          ...
          if (zone.geometry?.coordinates) {
            ...
            coords = (zone as any).coordinates;
          }
          const [lng, lat] = coords;

          const pm25 = zone.properties?.pm25 ?? zone.pm25 ?? 0;
          const name =
            zone.properties?.name || zone.name || zone.id || `Zone ${index + 1}`;
          const aqiCategory =
            zone.properties?.aqi_category || zone.aqi_category || "Unknown";
