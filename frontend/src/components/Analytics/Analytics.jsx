...
const Analytics = () => {
  const { measurements, dashboardData, aiAnalysis, loading, refreshSpecificData, triggerAIAnalysis } = useData();
  const [timeRange, setTimeRange] = useState('24h');
  const [selectedPollutant, setSelectedPollutant] = useState('pm25');
  const [chartType, setChartType] = useState('line');
  const [aiInsightType, setAiInsightType] = useState('comprehensive');
          ...
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                disabled={loading}
                className="bg-indigo-600 text-white p-2 rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center space-x-1"
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 border border-gray-200 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">AI Insight Type</label>
              <select
                value={aiInsightType}
                onChange={(e) => setAiInsightType(e.target.value)}
                className="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500"
              >
                <option value="comprehensive">Comprehensive</option>
                <option value="hotspots">Hotspot Analysis</option>
                <option value="warnings">Early Warnings</option>
        ...
        {aiAnalysis && (
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl shadow-lg p-6 border border-indigo-200 mt-6">
            <div className="flex justify-between items-center mb-4">
              ...
              >
                {showAIDetails ? 'Show Less' : 'Show More'}
              </button>
            </div>
            ...
            <div className="bg-white rounded-lg p-4 mb-4 border border-indigo-100">
              <div className="flex justify-between items-center">
                ...
                <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                  ...
                  {aiAnalysis.riskLevel?.toUpperCase() || 'UNKNOWN'} RISK
                </div>
              </div>
              <p className="mt-2 text-sm text-gray-600">{aiAnalysis.assessment}</p>
              ...
              <div className="mt-3 flex items-center text-xs text-gray-500">
                ...
                <span className="ml-2">{Math.round((aiAnalysis.confidence || 0) * 100)}%</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-lg p-4 border border-indigo-100">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  <div>
                    <span className="font-medium">Next 6 hours:</span> {aiAnalysis.predictions?.short_term}
                  </div>
                  <div>
                    <span className="font-medium">Next 24 hours:</span> {aiAnalysis.predictions?.daily}
                  </div>
                  <div>
                    <span className="font-medium">Next week:</span> {aiAnalysis.predictions?.weekly}
                  </div>
                </div>
              </div>
            </div>
            ...
            {showAIDetails && (
              <div className="mt-6">
                <div className="bg-white rounded-lg p-4 border border-indigo-100">
                  <h4 className="font-medium text-gray-900 mb-3">Health Risks & Recommendations</h4>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <h5 className="text-sm font-medium text-gray-700 mb-2">Immediate Health Risks</h5>
                      <ul className="space-y-1 text-sm text-gray-600">
                        {aiAnalysis.healthRisks?.immediate?.map((risk, index) => (
                          <li key={index} className="flex items-start space-x-2">
                            <span className="text-red-500">•</span>
                            <span>{risk}</span>
                          </li>
                        ))}
                        {(!aiAnalysis.healthRisks?.immediate || aiAnalysis.healthRisks?.immediate.length === 0) && (
                          <li className="text-green-600">No immediate health risks detected</li>
                        )}
                      </ul>
                    </div>
                    
                    <div>
                      <h5 className="text-sm font-medium text-gray-700 mb-2">Vulnerable Groups</h5>
                      <ul className="space-y-1 text-sm text-gray-600">
                        {aiAnalysis.healthRisks?.vulnerable?.map((group, index) => (
                          <li key={index} className="flex items-start space-x-2">
                  ...
                  <div className="mt-4">
                    <h5 className="text-sm font-medium text-gray-700 mb-2">Recommended Precautions</h5>
                    <ul className="space-y-1 text-sm text-gray-600">
                      {aiAnalysis.healthRisks?.precautions?.map((precaution, index) => (
                        <li key={index} className="flex items-start space-x-2">
                          <span className="text-blue-500">•</span>
                          <span>{precaution.replace('_', ' ')}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  
                  {aiAnalysis.healthRisks?.hospitalAlert && (
                    <div className="mt-4 bg-red-50 border border-red-200 rounded-md p-3">
                      <div className="flex items-center">
                        <AlertTriangle className="w-5 h-5 text-red-600 mr-2" />
                        <span className="text-red-700 font-medium">Hospital Alert</span>
                      </div>
                      <p className="text-sm text-red-600 mt-1">
                        Hospitals should prepare for potential increase in respiratory cases.
                      </p>
                    </div>
                  )}
                </div>
                
                <div className="bg-white rounded-lg p-4 border border-indigo-100 mt-4">
                  <h4 className="font-medium text-gray-900 mb-3">Trend Analysis</h4>
                  
                  <div className="flex items-center space-x-4">
                    <div className={`px-3 py-2 rounded-lg ${
                      aiAnalysis.trends?.direction === 'improving' ? 'bg-green-100 text-green-700' :
                      aiAnalysis.trends?.direction === 'worsening' ? 'bg-red-100 text-red-700' :
                      'bg-gray-100 text-gray-700'
                    }`}>
                      <div className="flex items-center space-x-2">
                        {aiAnalysis.trends?.direction === 'improving' ? <TrendingDown className="w-4 h-4" /> :
                         aiAnalysis.trends?.direction === 'worsening' ? <TrendingUp className="w-4 h-4" /> :
                         <BarChart3 className="w-4 h-4" />}
                        <span className="text-sm font-medium capitalize">
                          {aiAnalysis.trends?.direction || 'Stable'}
                        </span>
                      </div>
                    </div>
                    
                    <div className="text-sm text-gray-600">
                      <span className="font-medium">Timeframe:</span> {aiAnalysis.trends?.timeframe?.replace('_', ' ')}
                    </div>
                    
                    <div className="text-sm text-gray-600">
                      <span className="font-medium">Confidence:</span> {Math.round((aiAnalysis.trends?.confidence || 0) * 100)}%
                    </div>
                  </div>
                  
                  <div className="mt-4 text-sm text-gray-600">
                    <p>
                      {aiAnalysis.trends?.direction === 'improving' ?
                        'Air quality is showing signs of improvement. Continue monitoring to maintain progress.' :
                       aiAnalysis.trends?.direction === 'worsening' ?
                        'Air quality is deteriorating. Immediate action may be required to address pollution sources.' :
                        'Air quality conditions are relatively stable with no significant changes detected.'}
                    </p>
                  </div>
                </div>
              </div>
